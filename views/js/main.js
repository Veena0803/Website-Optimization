...
var resizePizzas = function(size) {
  ...
  function determineDx (elem, size) {
    ...
    var windowWidth = document.getElementById("randomPizzas").offsetWidth; // Changed querySelector to getElementById for the randomPizzas
  ...
  // Changed document selector to getElementsbyClassName inorder to optimize the loop
  //Store randomPizzaContainer in a global variable
  ...
  function changePizzaSizes(size) {
    var changedWidth; //variable definition to update values for pizza slider

    //switch case for the slider
    switch(size) {
      case "1":
        changedWidth = 25;
        break;
    ...
    //Moved the logic of randomPizzaContainer from below
...
//sets global animating object's value to false initially
window.animating = false;

//checks if global animating object is true
function pizzasAnimating() {
  if (!window.animating) {
    window.animating = true; //if not sets the animating object to true
//processes updatePositions function every time requestAnimationFrame results in true
...
function updatePositions() {
  ...
  // variable definition to optimize the updatePositions function
  ...
  var halfScreenWidth = ((window.innerWidth > 0) ? window.innerWidth : screen.width) / 2; // get window.innerWidth and divide the same by half to get halfScreenWidth
  var scrollArray = []; //store values from for loop in array

  for (var i = 0; i < 5; i++) {
    scrollArray.push(Math.sin((top / 1250) + i)); // Simplified the calculation 

  }

  //loop over length array and return result
  for (var i = 0; i < length; i++) {
    var phase = scrollArray[i % 5]; //Simplified the calculation
  ...
  //sets global object inside updatePositions() to an inital value of false
  window.animating = false;
}

// runs pizzasAnimating() on scroll
